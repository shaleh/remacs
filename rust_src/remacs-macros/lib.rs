...
use function::{Function, LispFnArgsRaw};

#[proc_macro_attribute]
pub fn lisp_fn(attr_ts: TokenStream, fn_ts: TokenStream) -> TokenStream {
    let fn_item: ItemFn = syn::parse(fn_ts.clone()).unwrap();
    println!("fn item: {:?}", fn_item);
    let function: Function = syn::parse(fn_ts.clone()).unwrap();
    println!("parsed fn: {:?}", function);
    let fnargs: LispFnArgsRaw = syn::parse(attr_ts.clone()).unwrap();
    println!("lisp fn args: {:?}", fnargs);
    // println!("attrs to parse: {:?}", attr_ts);
    // let parser = Punctuated::<Expr, Token![,]>::parse_terminated;
    // let foo = parser.parse(attr_ts.clone()).expect("punctuated");
    // // let parser = Attribute::parse;
    // // let foo: Vec<Attribute> = parser.parse(attr_ts).expect("lisp_fn parses");
    // println!("lisp fn args: {:?}", foo);
    fn_ts
