...
static Lisp_Object file_attributes_1 (int, char const *, Lisp_Object,
  ...
  for (struct dirent *dp; (dp = read_dirent (d, directory)); )
      ...
      if (wanted)
	{
	  ...
	  if (attrs)
		...
		= file_attributes_1 (fd, dp->d_name, directory, name, id_format);
  ...
  (Lisp_Object filename, Lisp_Object id_format)
{
  ...
  return file_attributes_1 (AT_FDCWD, SSDATA (encoded), Qnil, filename,
			    id_format);
}
#endif // IGNORE_RUST_POST

static Lisp_Object
file_attributes_1 (int fd, char const *name,
...
Lisp_Object
file_attributes (char const *name,
                 Lisp_Object dirname, Lisp_Object filename,
                 Lisp_Object id_format)
{
  return file_attributes_1(AT_FDCWD, name,
                           dirname, filename,
                           id_format);
}
